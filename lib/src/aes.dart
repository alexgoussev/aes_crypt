...
class _Aes {
  ...
  void aesSetKeys(Uint8List key, [Uint8List? iv]) {
    ...
    _aesIV = (iv == null || iv.isEmpty)? Uint8List(0) : Uint8List.fromList(iv);
  ...
  Uint8List aesEncrypt(Uint8List data) {
    ...
    final Uint8List encData = Uint8List(data.length); // returned cipher text;
  ...
  Uint8List aesEncryptBlock(Uint8List data) {
    ...
    final Uint8List encBlock = Uint8List(16); // 16-byte string
    int i;
  ...
  Uint8List aesDecryptBlock(Uint8List data) {
    ...
    final Uint8List decBlock = Uint8List(16); // 16-byte string
    int i;
  ...
  int _subWord(int w) {
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      _w = ((w << 8) & 0xFFFFFFFF) | _sBox[temp]; // add the substituted byte back
    }
    return _w;
  }
}
